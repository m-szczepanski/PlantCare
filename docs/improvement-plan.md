...
- `feature/watering-history-viz` — done (branch, PR pending). `WateringHistoryChart` on the detail card: dependency-free monthly bars (last 6 months, `chart-1` token) over the existing logs endpoint, sr-only data table, hidden when the window is empty. Test timeout raised for cmdk-heavy suites.
