...
- **Pickable soil mix catalog ships as `fix/soil-mix-picker`** — done. The plant-form soil mix was a free-text box; it is now a single-choice `Select` fed by a new DB-backed `SoilMix` catalog (`Id` + unique `Name`, `AddSoilMixCatalog` migration, provider-portable). Seeded idempotently by name from `Seed/soil-mixes.json` via `SeedLoader.LoadSoilMixesAsync` (12 common houseplant mixes), served at `GET /api/reference-data/soil-mixes` behind `SoilMixService` and consumed by `useSoilMixes`. The plant column stays a name **string** (not an FK) so legacy free-text and import documents remain valid; the picker renders a stored value missing from the catalog as an extra option (no silent data loss) and "No specific mix" clears it. No new env vars. **Superseded:** the two soil fields confused users at add time, so `SoilMix` was removed entirely (`fix/remove-soil-mix` below); `SoilType` is now the single soil concept.
- **Soil mix field removed ships as `fix/remove-soil-mix`** — done. `SoilType` (the functional field driving the watering-permeability factor) stays the single soil concept; `SoilMix` overlapped with it in the plant form and was cut end to end: plant column, `SoilMix` catalog entity/service/`GET /api/reference-data/soil-mixes`, `Seed/soil-mixes.json` + `SeedLoader.LoadSoilMixesAsync`, DTO fields (create/update/response/export), form picker, detail row, `useSoilMixes`, EN/PL keys, `SoilMixApiTests`. `RemoveSoilMix` migration drops the column and the `SoilMixes` table (data loss by design — the value had no functional consumer); legacy export documents with a `soilMix` property still import (unknown JSON fields are ignored). No new env vars.
