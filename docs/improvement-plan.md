...
- **Pickable soil mix catalog ships as `fix/soil-mix-picker`** — done. The plant-form soil mix was a free-text box; it is now a single-choice `Select` fed by a new DB-backed `SoilMix` catalog (`Id` + unique `Name`, `AddSoilMixCatalog` migration, provider-portable). Seeded idempotently by name from `Seed/soil-mixes.json` via `SeedLoader.LoadSoilMixesAsync` (12 common houseplant mixes), served at `GET /api/reference-data/soil-mixes` behind `SoilMixService` and consumed by `useSoilMixes`. The plant column stays a name **string** (not an FK) so legacy free-text and import documents remain valid; the picker renders a stored value missing from the catalog as an extra option (no silent data loss) and "No specific mix" clears it. No new env vars. **Superseded:** the two soil fields confused users at add time, so `SoilMix` was merged into `SoilType` as selectable blend presets (`fix/remove-soil-mix` below); `SoilType` is now the single soil concept.
- **Soil fields merged into one ships as `fix/remove-soil-mix`** — done. The plant form's two soil fields (`SoilType` + `SoilMix`) overlapped; `SoilType` — the functional one driving the watering-permeability factor — stays as the single soil concept. The old soil-mix catalog names were **merged into the soil-type picker**, not dropped: `SoilTypes` now owns a `Mixes` list per type (the 12 former catalog blends mapped onto the 6 permeability categories, e.g. "Aroid chunky blend" → `ChunkyBark`), served on `GET /api/reference-data/soil-types` as `mixes` and rendered as selectable presets under each type in the plant-form `Select` (choosing a blend stores its type, so the schedule math keeps its single source of truth). Cut end to end: the `SoilMix` plant column, catalog entity/service/endpoint/seed + `SeedLoader.LoadSoilMixesAsync`, DTO fields (create/update/response/export), form picker, detail row, `useSoilMixes`, EN/PL keys, `SoilMixApiTests`. The `RemoveSoilMix` migration **backfills `SoilType` from known `SoilMix` names** (SQL `CASE`, only where `SoilType` is null; unrecognized legacy free-text stays null) before dropping the column and the `SoilMixes` table. Legacy export documents with a `soilMix` property still import (unknown JSON fields are ignored). No new env vars.
- **Epic 10 ships as one PR: `epic/health-checkups`** — all three chunks done.
  - `feature/health-check-model` — `HealthStatus` enum (`Sick`/`Bad`/`Good`/`Excellent`) + immutable `PlantHealthCheck` history and current-state columns on `Plant` (`HealthStatus`, `LastCheckupAt`, plus the reminder stamp `CheckupReminderSentAt`; `AddHealthCheckups` migration, provider-portable). `HealthPolicy` is the single source for the 30-day period and the factors, applied inside `WateringScheduleService` (watering: sick ×1.5, bad ×1.25, good ×1, excellent ×0.8 after the soil factor, before winter doubling; fertilizing: excellent ×0.8, sick/bad **paused** by clamping the due date out to the next checkup day — the soil-wet pattern — with a `hint.fertilizingPaused` care-task hint; repotting untouched). `GET/POST /api/plants/{id}/health-checks` (POST returns the rescheduled plant; no edit/delete — history is immutable), `healthStatus`/`lastCheckupAt`/`checkupDue` on plant + dashboard DTOs, export/import carry the status and history (schema stays v1-compatible).
  - `feature/health-checkup-ui` — `HealthBadge` (icon+text, not color alone) on `PlantCard` + a "Checkup due" flag line; `HealthCheckCard` on detail: current status, due prompt, optional note, one-tap four status buttons, checkup history; `CheckupBanner` on the dashboard (pluralized count + chip links to due plants); `useHealthChecks`/`useAddHealthCheck` (refreshes detail/list/dashboard/care-tasks caches — the schedule visibly shifts on answer); EN/PL catalogs.
  - `feature/health-checkup-reminders` — the daily `WateringCheckJob` grows a "Health checkups due (N):" section riding the watering digest, or its own digest ("N health checkups are due") when nothing needs water; per-plant 30-day dedup stamped on `CheckupReminderSentAt` only after a successful delivery (failures re-try next run); muted and snoozed plants excluded; ntfy/Telegram failure keeps degrading gracefully.
  Deviation: the effective status is **mirrored** on `Plant` (`HealthStatus` + `LastCheckupAt`) rather than derived per-read from the history table — the schedule service, dashboard and digest all need it on every plant and the mirror keeps `WateringScheduleService.GetDueInfo(task, plant)` signature-free (history stays the audit source; `checkupDue`/factors read the mirror). Health factor values are backend-owned (`HealthPolicy`), deliberately not mirrored client-side (no health control in the plant form). No new env vars.
- **Test notifications + manual digest trigger ships as `feature/notifications`** — done. On-demand verification so a fresh install can confirm end-to-end delivery without waiting for the `0 8 * * *` cron or forcing a plant overdue.
  - Backend: `INotificationTestService` publishes a localized test push (`test.title`/`test.body`, EN/PL) through every registered `INotificationChannel` and reports per-channel `delivered`/`error` (a throwing channel is caught per-channel, never aborts the rest or the request); thin `NotificationsController` exposing `POST /api/notifications/test { message? }` and `POST /api/notifications/run-check` (runs the real `WateringCheckService.RunAsync` immediately, honoring the once-per-day digest dedup). `GET /api/status` now also returns the list of configured `channels` (`StatusInfo.Channels`). No new env vars, no schema change.
  - Frontend: `useNotifications` (`useSendTestNotification`/`useRunWateringCheck` TanStack mutations) drive new **Send test notification** (optional custom message `Input`) and **Run watering check now** buttons plus a delivery-channels badge row on the Status page; success/partial/failure toasts reflect per-channel results and re-run invalidates the status query; EN/PL catalogs. Deviation: the docs' phone-subscription guidance was expanded to call out that the ntfy mobile app defaults to public `ntfy.sh` and must be pointed at a self-hosted server (`NTFY_PUBLIC_PORT`, e.g. `7272`).
