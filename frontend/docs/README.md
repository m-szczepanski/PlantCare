...
- `PlantPhoto` renders the profile `PhotoUrl` (lazy, `alt` = nickname) on cards and detail, swapping in a dashed placeholder illustration when there is no photo or the image fails to load (`onError` fallback). Uploaded photos live under `/uploads/...` (served by nginx from the shared `plant-photos` volume); the detail page's "Upload photo" button posts multipart to `POST /api/plants/{id}/photo` via `useUploadPlantPhoto`. The URL field in the plant form remains for externally hosted photos. On the detail page the photo sits to the **right** of the Details card on `lg+` and is **exactly as tall as the card**: a `relative` wrapper carries the layout (`lg:self-stretch`, no fixed height) and the `PlantPhoto` is `absolute inset-0 h-full w-full` with `object-cover` cropping — the absolute image contributes no intrinsic height, so the card defines the row and the photo fills it (class-based stretch on the replaced `<img>` itself proved unreliable in Chrome; measured via headless Chrome to keep them equal). Stacked on top with a `3/4` portrait ratio on mobile.
...
- `PlantCard` layout: a full-width `PlantPhoto` banner across the top (`aspect-[16/7]`, edge-to-edge via the card's `overflow-hidden`), then name (`text-xl`), room line (compass icon + light-match badge when set), species line (with the toxicity warning icon), and the badge row: `DueStatusBadge` (short status **text + icon** — accessible without relying on color) plus `HealthBadge`/checkup prompt when present. `DueCount` — days-until-due as the primary number ("3 / days overdue" red, "Now / due today", "4 / days to go", "— / not scheduled") — sits top-right next to the name. A `mt-auto` footer block (top border) carries two icon rows — relative "Watered 3 days ago" (`src/lib/dates.ts`, with a "Soil wet — recheck {date}" suffix while `soilWetUntil` is active) and "Next due {date}" (falling back to "Every N days" / "No watering schedule") — with the Water button beneath; cards in a grid stretch to equal height (`h-full` card + `li`).
- Health checkups (Epic 10): `HealthBadge` maps the `HealthStatus` enum to a variant+icon pair (Sick/HeartCrack destructive, Bad/ThumbsDown outline, Good/ThumbsUp secondary, Excellent/Sparkles default — icon **and** text, never color alone). `HealthCheckCard` on the detail page shows the current status + "Last checkup {date}" (or "no checkup yet"), a `role="status"` due prompt while `checkupDue`, an optional note field and the **one-tap four buttons** (order Sick→Excellent from `HEALTH_STATUSES` in `src/lib/health.ts`) posting `POST /api/plants/{id}/health-checks` via `useAddHealthCheck`, plus the checkup history (everything after the newest entry) — logging a checkup also refreshes the care-tasks card since the schedule shifts. `CheckupBanner` sits under the stats strip on the dashboard: title + pluralized "N plant(s) are due…" + one chip-link per due plant to its detail page. All strings under `health.*` (EN/PL), toasts under `toasts.checkup*`.
...
- Loading uses card-shaped skeletons, never plain text: `PlantCardSkeleton` (mirrors the card's banner-photo + body + footer shape) / `PlantCardSkeletonGrid` for the dashboard and plants list, `PlantDetailSkeleton` for the detail page. Each exposes a `role="status"` container with an sr-only "Loading…" label and `aria-hidden` placeholder cards.
...
- `src/components/AppShell.tsx` is the single layout wrapper (used by `App.tsx`): a persistent shadcn sidebar with the brand and the main nav (Dashboard / Calendar / Insights / Rooms / Profiles / Status / Plants / Add plant), an active-item state driven by the route, a sticky `h-16` top bar on mobile (sidebar trigger + brand), and the theme + language toggles. The page column is a generous `max-w-7xl` (`px-4 sm:px-6 lg:px-8`) so the tile grids breathe on wide screens, and page titles are `text-3xl`. `/wall` deliberately renders outside the shell.
...
| View | Contents |
|------|----------|
| Dashboard | Stats strip (overdue / due today / total, green "all caught up" state) + monthly health-checkup banner (chips linking to due plants) + "due today / overdue / upcoming" summary, toggleable to a group-by-room view (`?group=room`) |
...
| Rooms | Room CRUD (`/rooms`): name, orientation, optional environment (light exposure / humidity / temperature °C). Each tile leads with a three-cell stat strip (total plants / overdue / due today, computed from the room's plants), an always-present environment chip row (light / humidity / temp, each with a "not set" fallback), and a plant list (photo avatar, name, light-match badge, relative watered line, capped at 5 with a "+N more" line) or a dashed "No plants here yet" empty block with a Browse action; actions sit in a bordered footer pinned to the bottom (`mt-auto`) so tiles stay full-height and uniform in the grid |
| Plant detail | Header with the name + due badge and the action buttons directly beneath ("Mark as watered", upload/change photo, Edit, Back to list, Delete). Below it a two-column hero: the Details card (room + light-match badge, species, acquired, last watered, pot size, soil type, propagated-from, watering interval, next due, reminders, vacation snooze, soil-wet deferral) on the left and the photo on the right, stretched to the card's full height with `object-cover` cropping (stacked above the details as a 3/4 portrait on mobile). Then (full width) toxicity badges, care tips, diagnostics checklist, "log a watering" form (note + optional amount/method), care-tasks card, health checkup card (current status + one-tap 4-status checkup + history), care journal, notes, and the watering history with monthly bar chart |
...
All endpoints under `/api/*` — plants CRUD, rooms CRUD (`GET/POST/PUT/DELETE /api/rooms`), `POST /api/plants/{id}/water`, `POST /api/plants/{id}/photo` (multipart upload), `GET /api/plants/{id}/watering-logs` (detail page history), `POST`/`DELETE /api/plants/{id}/soil-wet` (watering deferral), `GET`/`POST /api/plants/{id}/health-checks` (monthly checkup history + one-tap answer), plant-profiles list/create/update, the dashboard summary, and reference data (`GET /api/reference-data/soil-types` for the plant-form soil picker). Shapes are defined by the backend's DTOs (see `backend/docs/README.md`). Uploaded photos are static files served by nginx under `/uploads/*`, outside the API surface.

## Conventions

- Functional components only.
- Server state via hooks (TanStack Query); local UI state stays in the component.
- `useWaterPlant` is optimistic: `onMutate` patches the cached plant (list + detail) **and the dashboard buckets** (`["dashboard"]`, keeping the plant in sync across pages) with the computed "just watered" state, failures roll both caches back, and the success toast carries an **Undo** action calling `DELETE /api/plants/{id}/water`. `useBulkWater` optimistically moves the watered plants out of overdue/due today the same way. All plant mutations (`create`/`update`/`delete`, snooze, soil-wet, health checkup) also invalidate `dashboardKeys.all` — `useDashboard` exports `dashboardKeys` as the single source for that query key — so a watered plant disappears from "Due today" on the dashboard instantly, without a refetch. `useAddHealthCheck` sets the returned plant into the detail cache and re-buckets the dashboard (a sick answer can push a due-today plant to upcoming) and refreshes the care-tasks + health-check history queries. Destructive actions (delete plant) go through a shadcn `alert-dialog` confirm, never `window.confirm`.
- Cross-section reactivity comes from shared query keys, not per-page refetches: the Calendar projects over the same `["plants"]` cache (`usePlants`), so an optimistic water/bulk-water updates its due markers too. Room mutations additionally invalidate `["plants"]` and `dashboardKeys.all` (room names appear on plant cards/dashboard), and plant-profile updates invalidate `["plants"]` + `["dashboard"]` since a profile supplies the watering interval, care tips, and toxicity shown on plants. Read-only derived queries (status, insights) refetch on mount via the default `staleTime: 0`; reference data (soil types) is immutable at runtime and cached up to an hour.
- Compose shadcn primitives into app-specific components rather than editing generated files.
- Design-system enforcement runs through `npm run lint` (oxlint + `@shadcn/lint`, config in `.oxlintrc.json`): `no-restyle` (layout allowed globally; per-component contracts encode the intentional class-passing sites), `no-raw-colors`, and `no-arbitrary-values` (warn). Generated files under `src/components/ui/**` are exempt from the restyle rules; `no-unknown-classes` is off (needs Tailwind v4). Widen a contract before ever restyling against one.
