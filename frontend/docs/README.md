...
- Soil mix picker: the `SoilMix` field is a single-choice `Select` fed by the DB-backed catalog `GET /api/reference-data/soil-mixes` (`useSoilMixes`), storing the mix **name** string (server keeps accepting any value for import/legacy compatibility). A plant whose stored mix is not in the catalog gets it prepended as an extra option, so editing never silently drops the value; "No specific mix" clears it.
...
All endpoints under `/api/*` — plants CRUD, rooms CRUD (`GET/POST/PUT/DELETE /api/rooms`), `POST /api/plants/{id}/water`, `POST /api/plants/{id}/photo` (multipart upload), `GET /api/plants/{id}/watering-logs` (detail page history), `POST`/`DELETE /api/plants/{id}/soil-wet` (watering deferral), `GET`/`POST /api/plants/{id}/health-checks` (monthly checkup history + one-tap answer), plant-profiles list/create/update, the dashboard summary, and reference data (`GET /api/reference-data/soil-types` for the plant-form soil picker, `GET /api/reference-data/soil-mixes` for the soil-mix catalog). Shapes are defined by the backend's DTOs (see `backend/docs/README.md`). Uploaded photos are static files served by nginx under `/uploads/*`, outside the API surface.
